...
<!--
Bridge section (~8 min, zero keyboard) between "what is a cloud" and the stack. The table you just showed is a pile of primitives: powerful, and completely undifferentiated. This section names the discipline the workshop actually teaches: platform engineering. And it grounds it in Nav, a real Norwegian national-scale example, so nobody leaves thinking today was a laptop trick.
...
A platform is <strong>opinions about how primitives compose</strong>. The discipline is making the right thing the easiest thing.
</div>
...
Hans speaks from experience here. This is the "I work on this" slide, and the credibility anchor for the whole day: platform engineering isn't a conference fashion, it's how a critical national institution ships software.
...
- Golden path: nais.yaml's core idea, a manifest that declares dependencies and not just the deployment, is precisely the Application XR of module 04/09. Say it explicitly: "you are building a build-your-own-Nais today."
...
- Secure defaults: at Nav, apps can't talk to each other until an access policy says so. Making the secure path the default path is the deepest lesson. The security adventure lets people feel it by breaking a live pipeline with default-deny and earning it back.
