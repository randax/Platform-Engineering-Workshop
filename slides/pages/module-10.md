...
Step 3, Agent: only when the hint runs out does "Open investigation" appear on the application-detail page, and it's always available, never mandatory. Most incidents resolve at step 1 or 2; that's the ladder working as designed, not the agent failing to add value.
...
And it arrives exactly the way every capability has all day: copy gitops/catalog/kagent.yaml to gitops/apps/, push to Gitea, ArgoCD syncs it. No new mental model for "how do I turn this on"; the progressive-enable mechanic from module 02 keeps paying off.
