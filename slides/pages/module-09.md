...
<div class="grid grid-cols-2 gap-4 mt-4">
  ...
  <div class="principle">
    ...
    <div class="tie" style="opacity:.85">One controller drives a <em>defined sequence</em>: step 1→2→3, retries, a visual DAG. Door 0 builds a real one, your CI pipeline.<br><b>= Argo Workflows</b> &nbsp;(≈ Step Functions)</div>
  </div>
</div>
...
Orchestration: a central workflow drives an explicit sequence with retries, branching, and a visual execution graph. Easier to reason about and observe; more coupling to the orchestrator. AWS Step Functions is the reference; the open analog is Argo Workflows, which you ALREADY ran in module 07: your in-cluster CI build is an Argo DAG. Same engine, same visual graph, no new tool.

The takeaway: you don't pick a winner. Mature platforms offer both, and the skill is choosing. (We deliberately skipped a dedicated orchestration module; it would be a tangent in a platform-assembly workshop. But the engine and the concept are both already here.)
-->
...
2. The storage view: the Gallery (refresh) shows the thumbnail and its metadata; raw S3 shows originals/, thumbs/, and meta/<key>.json in the images bucket, module 03 muscle memory with s5cmd against s3.cloudbox.k8s.test.
