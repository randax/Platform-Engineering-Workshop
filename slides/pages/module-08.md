...
Walk the architecture in one breath. internal/kube/client.go authenticates with nothing but the pod's mounted ServiceAccount token. A scoped read-only ClusterRole covers just the surfaces it renders (ArgoCD apps, CNPG clusters, ksvcs, pods/nodes/events/workloads), not read-all; check it with kubectl describe clusterrole portal-read. internal/kube/resources.go lists ArgoCD Applications, CNPG Clusters, and Knative Services as dynamic resources. The "New database" form POST in internal/web/databases.go builds a WorkshopDatabase object and creates it, about 20 lines that replace a whole portal product's scaffolder, because module 04's XRD and Composition already did the hard part.
...
<!--
The build-vs-buy interlude. Be honest in both directions: "bespoke always" is as wrong as "Backstage by default".
