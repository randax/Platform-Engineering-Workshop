...
| `kind-fallback.sh` | Plan B when neither substrate runs: same cluster shape on kind + Cilium, the **same** `cilium_ingress_values` (lib.sh) as the docker substrate, host port 80 → the ingress NodePort and the same marked `/etc/hosts` block — so modules 02 onward are identical. Loses the Talos content, gains robustness. Writes no `~/.cloudbox/substrate`: kind is not a substrate |
| `kind-fallback.sh --delete` | Deletes that cluster **and** removes the `/etc/hosts` block. `destroy-cluster.sh` cannot — it reads `~/.cloudbox/substrate` and knows only tbx and docker |
