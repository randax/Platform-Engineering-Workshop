...
./scripts/collect-debug.sh          # bundle up what's wrong, to file an issue
```
...
| `collect-debug.sh [module]` | Read-only: writes one redacted markdown bundle to `~/.cloudbox/debug-<ts>.md` — platform, tool versions, substrate, cluster/ArgoCD state, logs of every not-Running pod, optionally a module's `verify.sh`, and the preflight — then prints the prefilled help form and the `gh issue create` line. Sends nothing itself. Consults the context guard non-fatally: a wrong context is *reported* in the bundle instead of ending it, and no cluster is asked anything until the guard passes |
