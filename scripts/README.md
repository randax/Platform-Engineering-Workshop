...
| `kind-fallback.sh --delete` | Deletes that cluster, removes the `/etc/hosts` block (only when the recorded identity is `kind`) and clears the identity. `destroy-cluster.sh` cannot — it builds and tears down substrates, and kind is not one |
