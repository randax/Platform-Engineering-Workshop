...
What happens when you can no longer trust your cloud provider? Its pricing, its
jurisdiction, its roadmap? In this workshop you build the answer: a complete
...
and a GitOps engine delivering the entire platform on top. You get real VMs via
...
portal (source in `apps/`) that surfaces everything you built and lets you self-service
a database from a form.
...
<p align="center"><em>Left: a component's live Monitoring detail, with CPU/memory sparklines and a log tail from the OTel stack. Right: the Components health page. Both shown in dark mode; the console ships light + dark.</em></p>
...
serves it to the VMs at the cluster gateway). A tbx laptop needs no Docker at all. On
the tbx path step 2 also warms the Talos disk image (`tbx cache pull`, 95 MB on arm64 /
204 MB on amd64), and step 3 asserts a complete `disk.raw` is in `~/.talosbox/cache` and
grades the images with `tbx cache warm --check` (`--check --deep` before you travel).
At the venue `tbx mirror offline on` stops tbx's mirror from fetching upstream itself, so
a missing image surfaces as a mirror miss instead of being quietly filled over the WiFi;
the nodes keep `skipFallback: false`, so the pull then goes direct, slowly and visibly.
One trade-off to know: tbx's store serves VMs only, so on a tbx laptop
...
been created, `~/.cloudbox/substrate` is a *record*, not a preference. `create-cluster.sh`,
...
On a machine with no record (a fresh laptop, CI) the override is simply the answer.
...
You'll run these together with us; no need to run them at home. (You can, though: the
whole workshop works offline once the images are pre-pulled and the Helm charts are
vendored, and they are, in `scripts/manifests/`.)
...
finishes either way and says which lines are still there. On the tbx substrate nothing
touches `/etc/hosts`; talos-box's own resolver answers the names.
...
Docker-substrate machine cannot take out a live cluster's names. It clears the
...
the lesson.** Docker won't start, a mise shim misbehaves, a download died halfway? Sic
your agent on those with everything it has. Yak-shaving is nobody's learning objective.
The platform concepts are.
...
which is somewhere else entirely, needs the Ports tab.
