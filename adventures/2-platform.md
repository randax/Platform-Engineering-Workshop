...
1. Vendor the upstream static manifest into `gitops/components/cert-manager/`
   ...
   `catalog/cert-manager.yaml`, enable it. It's CRD-heavy, so you'll want
...
door is about what operating Kafka *costs*, exactly what a platform team must
know before saying yes to it.
...
Pick a component nobody chose today: Keycloak, external-secrets, a Valkey
operator. Vendor it and take it all the way to an XR. The catalog mechanic is
the platform's extension API; it doesn't care what you feed it.
