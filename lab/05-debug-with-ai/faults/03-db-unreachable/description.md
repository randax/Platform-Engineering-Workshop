...
1. `kubectl -n faultlab-03 logs deploy/orders-api` shows errors, but the target pod is
   healthy.
2. Rule out the usual suspects: `kubectl -n faultlab-03 get endpoints inventory-db`
   ...
   (resolves!). Healthy pod, endpoint, DNS, and still a *timeout*: suspect policy.
...
5. Watch the verdict happen (the smoking gun, straight from the eBPF datapath):
   ...
   ```
   This prints `Policy denied ... faultlab-03/orders-api -> faultlab-03/inventory-db ... 5432`.
