...
# Every pin, including the NodePorts these helpers reach for, comes from the
# single pin source.
# shellcheck source=../scripts/versions.env
source "${LAB_COMMON_DIR}/../scripts/versions.env"

# The workshop-context guard is DEFINED in scripts/context-guard.sh — one copy,
# shared with scripts/lib.sh so bootstrap-gitops.sh, seed-gitea.sh and catch-up.sh
# are held to the same rule. Read its header for why it refuses rather than warns.
# Sourcing it only defines the guard; the call below is what makes it fire on
# every lab script, and it stays HERE (not in the shared file) because
# create-cluster.sh legitimately runs before any workshop context exists.
# shellcheck source=../scripts/context-guard.sh
source "${LAB_COMMON_DIR}/../scripts/context-guard.sh"
