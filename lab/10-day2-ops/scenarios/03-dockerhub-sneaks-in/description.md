...
1. `kubectl -n demo get pods -l app=demo-web` shows healthy pods. Do not stop here; a
   green pod list is not evidence that the last release was good.
2. `kubectl -n demo get pods -l app=demo-web -o jsonpath='{.items[*].spec.containers[*].image}'`
   shows the registry host is now `docker.io`.
...
4. Prove it from the mirror side, on your host (docker/kind):
   ...
   returns `200`. Your mirror carries that path, whatever registry the manifest names,
   which is exactly why the change was invisible.
