...
1. `kubectl -n demo get all` shows the Deployment stuck and the newest pods restarting.
2. `kubectl -n demo describe pod <new-pod>` shows Events with the repeated restarts and
   `CrashLoopBackOff` backoff.
3. `kubectl -n demo logs <new-pod> --previous` shows the process reporting the listen
   error for `:8080-canary` immediately before exiting.
