...
- Cilium install: machine-config patch `cluster.network.cni.name: none` (+ optionally
  ...
  removes a nested-cgroup variable, the robustness vs. wow-factor tradeoff.
- Cilium stable 1.20.0; Talos docs use 1.18.x, either fine. Kernel needs ≥5.10:
  ...
  platform for this combo and must be tested explicitly**. Linux: warn about firewalld/nftables
...
- Talos v1.13.8 ships Kubernetes 1.36.2, which satisfies Knative's ≥1.34 requirement.

## 2. Object storage: RustFS is a **conditional GO**, SeaweedFS is Plan B
...
- **Switch triggers to SeaweedFS 4.x** (`allInOne.enabled` single pod, ~500 MB, weekly
...
- v3 vs v2 gotchas for demos: logs RBAC enforced by default (read-only demo users need
  ...
  argocd-autopilot is stale, avoid it.
- **App-of-apps is the right bootstrap for one cluster with disparate components**
  ...
  in `argocd-cm`** (removed in 1.8; without it waves don't gate, the most-missed step);
...
- **Catch-up mechanism:** per-module script force-pushes the canonical module-N state to
  ...
  fix a broken cluster, so also ship a nuke-and-rebuild-to-checkpoint script
...
- **Crossplane v2.3.4** (v2 GA Aug 2025; CNCF **graduated** Nov 2025). Teaching deltas:
  ...
  (native P&T removed); **compositions emit arbitrary k8s resources directly; the official
  docs compose a CloudNativePG `Cluster` as their example**. No provider-kubernetes wrapping.
...
- **Knative Serving v1.23.0** (released; pinned together with Eventing v1.23.0). Kourier ingress, halve the
  default requests (official k0s blog pattern) to land at ~0.6 GiB. Set
  `registries-skipping-tag-resolving` for the local registry. Avoid Gateway API mode with
  Cilium (not in conformance matrix). No published Talos+Knative report; smoke-test it.
...
+ OS/browser/IDE, for a landing zone of
...
- **Pacing reality:** plan ~5 exercises, expect to finish 3. 45–60-min modules: short intro,
  demo, a 10–15-min lab, then walk the solution to re-sync. Hands on keyboards within the
  first 10 minutes. ≥50% hands-on. Bonus challenges for the fast 20%.
...
- Proven patterns (all with verified exemplars):
  - **Broken-on-purpose labs**: SadServers, Klustered, vellankikoti's 35 k8s failure
    ...
    workshop.
  - **Goal-oriented labs with automated checkers**: the iximiuz/Killercoda model of outcome +
  ...
  - **Layered free hints**: the eficode kubernetes-katas template of overview bullets, steps
