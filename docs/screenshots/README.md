...
| `console-applications.png` | The **Applications** page — the golden path (PRD-0003) as a console action: deploy an app + its Postgres database + S3 bucket from one `Application` XR, with per-row Ready/URL/Delete, plus **Redeploy** on source-built apps (rebuild the repo → roll forward, PRD-0012) (#28). When an app is unhealthy it also shows **Diagnostics** — the failing condition inline plus the pod's `ImagePullBackOff`/crash cause (DR-0005). |
