...
- The **CNCF Platforms White Paper** gives the vendor-neutral definition: seven platform
  attributes, a 13-capability model, and a maturity model.
- **Team Topologies** (Skelton & Pais) is the intellectual origin: platform-as-a-product,
  the Thinnest Viable Platform (TVP), and reduced cognitive load as the platform's
  primary purpose.
- **AWS Prescriptive Guidance** and **platformengineering.org** operationalize the same
  principles: golden paths, self-service, optional adoption, onboarding.

We anchor on the CNCF seven attributes because they are vendor-neutral and the
closest thing to an official definition.

## The seven attributes in this reference implementation

| CNCF attribute (paraphrased) | Where the attendee builds it |
|---|---|
| **Platform as a product**: designed and evolved around user needs, like any product | The Cloudbox Console, a real front door you can read every line of (module 08) |
...
| **Self-service**: request and receive capabilities autonomously and automatically | Crossplane XRs plus the console's New Database / New Function forms (modules 04 / 08) |
| **Reduced cognitive load**: an essential goal | T-shirt sizing: `size: small`, not twelve CNPG fields (PRD-0006 / #81) |
| **Optional & composable**: use only the parts you need | Catalog-enabled capabilities plus a `kubectl` escape hatch; core is 5 modules, the rest optional |
...
Team Topologies defines the TVP as "the smallest set of APIs, docs and tools that
accelerates the teams consuming it". That is the workshop's whole shape in one phrase:
