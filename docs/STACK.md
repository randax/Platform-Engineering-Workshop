# The stack: component choices and tradeoffs

The reference behind the workshop's [stack slides](../slides/pages/stack.md).
For each platform component: what we run, what we rejected, and the tradeoff we
...
| Storage class | **local-path-provisioner** v0.0.37 | Longhorn · Ceph/Rook CSI | Node-local hostPath, one Deployment; no replication or snapshots needed for one node. |
| Self-service API | **Crossplane** v2.3.4 | Helm/operators · Crossplane v1 | Namespaced XRs (no Claims) compose arbitrary K8s resources directly; one YAML yields the whole stack. |
...
| In-cluster CI | **Argo Workflows** v4.1.1 + **BuildKit** v0.32.2 | Tekton · external/cloud CI | Rootless in-cluster image builds into your own Zot, no cloud minutes; Kaniko was archived June 2025. |
| Durable messaging | **NATS** 2.14.5 + JetStream | Kafka · RabbitMQ | Durable streaming in ~15 MB of Go vs. GBs of JVM/Erlang + coordination. |
...
| Dashboards | **Grafana** 13.1.3 | (part of the LGTM blob) | Native MetricsQL/LogsQL datasource plugins baked into the image, nothing fetched at boot, so it stays offline. |
...
kubeadm and minikube give you a general-purpose distro you must harden and that can drift.
kind gives you a mutable node you can shell into, which is exactly what Talos refuses to be.
The tradeoff is deliberate: you give up the SSH escape hatch to get zero drift and a tiny
attack surface. Pinned v1.13.8, never 1.12.x (`cni: none` docker clusters hung on readiness
...
That's a robustness-vs-wow dial, stated honestly.
...
multi-controller setup that's less legible to teach in one sitting; raw `kubectl` has no
...
the 262 KB annotation limit, hence `ServerSideApply=true`). Crossplane's compositions compose
a CNPG `Cluster` directly; the official Crossplane v2 docs use exactly this example.
...
which is the exact roadmap-capture risk this workshop is about. State it correctly: MinIO
never relicensed, and RustFS is an *independent* Apache-2.0 reimplementation, not a MinIO
"successor". The tradeoff is maturity: RustFS is at 1.0.0-rc.2, presigned GET/PUT work but
...
It replaced the AWS CLI on 2026-08-17 (129 MiB of pre-pull down to 12 MiB, and no
AWS-branded CLI in a workshop about not using AWS; see [HAZARDS.md](HAZARDS.md)).
...
a bucket Job and a Knative `Service` with no `provider-kubernetes` `Object` wrapping.
...
directly, so it needs an aggregated ClusterRole per composed third-party API group,
and budgets ~0.7–1 GiB. The Function package is fetched from the registry (not the node image
...
Kourier is the ingress (Gateway API mode is deliberately avoided; it's not in Cilium's
conformance matrix). The in-memory channel is not durable, by design. This is a 4-hour lab,
not Kafka school.
...
offline. Kaniko was archived June 2025, so BuildKit is the 2026 answer. Tekton has a
heavier CRD footprint; external/cloud CI needs internet and a cloud account, defeating the
...
ecosystem and throughput for a fraction of the RAM. This is durable messaging in its
smallest useful form, sized for a laptop. It's the durable counterpart to the capstone's
in-memory broker and the queue the golden-path `Application` XR requests via `spec.queue`.
...
receivers the core image lacks): an agent DaemonSet (pod logs to VictoriaLogs, kubelet
stats to VictoriaMetrics) and a gateway Deployment (`k8s_cluster` + prometheus scrape to
...
VictoriaLogs (`:9428`, Loki API, replaces Loki), and VictoriaTraces (`:10428`, Jaeger
...
- **Pre-pulled and offline.** `cloudbox-init.sh` pulls the pinned list into a local mirror;
...
- **Fits a 16 GB laptop.** In-cluster total ≈ 7.5–8 GB (Talos 2-node ~1.7, Cilium
  ...
  ~1 on-demand) plus Docker VM + OS/browser/IDE. Hence the published spec: 16 GB minimum
...
and some picks flip: local-path becomes a real CSI, RustFS becomes MinIO or S3, in-cluster
Gitea becomes your actual GitHub/GitLab, plus cert-manager and the OTel Operator. The shape
stays identical; the parts change with the constraints. That is the skill worth taking home.
