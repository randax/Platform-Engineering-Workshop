...
enough* IdP, with its own login page and user store, so attendees log in with no
...
> Run a *standalone* Dex; don't try to reuse the one ArgoCD bundles, since that
