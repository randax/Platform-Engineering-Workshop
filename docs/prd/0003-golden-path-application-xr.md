...
XR, one YAML in and a managed Postgres + bucket out, and it's the highlight of the
day. But a real internal developer platform (IDP) doesn't stop at a database: you
declare a whole **application**, with its image, URL, database, bucket and queue, and the
...
- *Platform-service URLs (optional polish, separable):* give ArgoCD/Gitea/console
  ...
  the CNI, which keeps the eBPF story intact). Tracked as a stretch, not required for the
  golden path.
...
- **RAM/components:** ~zero new; it reuses CNPG, RustFS, Knative, NATS. Cilium Gateway
...
- **Scope creep toward "real Nais"**: resist it; the teaching win is the *shape*, not
  feature parity.
...
1. Land NATS (PRD-0001) as a standalone component.
